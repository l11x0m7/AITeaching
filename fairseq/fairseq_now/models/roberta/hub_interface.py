...
class RobertaHubInterface(nn.Module):
    ...
    def extract_features(self, tokens: torch.LongTensor, return_all_hiddens: bool = False) -> torch.Tensor:
        ...
        features, extra, last_attn = self.model(
        ...
        if return_all_hiddens:
            ...
            return [inner_state.transpose(0, 1) for inner_state in inner_states], last_attn
        else:
            return features, last_attn  # just the last layer's features
    ...
    def predict(self, head: str, tokens: torch.LongTensor, return_logits: bool = False):
        features, last_attn = self.extract_features(tokens)
        logits = self.model.classification_heads[head](features)
        if return_logits:
            return logits, last_attn
        return F.log_softmax(logits, dim=-1), last_attn
    ...
    def extract_attention_to_words(self, sentence: str, sentence2: str, features) -> torch.Tensor:
        ...
        s2_bpe_toks = torch.cat([bpe_toks[0:1], bpe_toks[s1_bpe_len + 1:]], dim=0)
        features = torch.cat([features[0:1], features[s1_bpe_len + 1:s1_bpe_len+s2_bpe_len]], dim=0)
        ...
        alignment = alignment_utils.align_bpe_to_words(self, s2_bpe_toks, spacy_toks_ws)
        aligned_attn = alignment_utils.align_features_to_words(self, features, alignment)
        # print(spacy_toks_ws)
        # print(aligned_attn)
    ...
    def extract_features_aligned_to_words(self, sentence: str, return_all_hiddens: bool = False) -> torch.Tensor:
        ...
        features, last_attn = self.extract_features(bpe_toks, return_all_hiddens=return_all_hiddens)
        features = features.squeeze(0)
        last_attn = last_attn.squeeze(0)
        aligned_feats = alignment_utils.align_features_to_words(self, features, alignment)
        aligned_attn = alignment_utils.align_features_to_words(self, last_attn, alignment)
        ...
        doc.user_token_hooks['attn'] = lambda token: aligned_attn[token.i]
        return doc
